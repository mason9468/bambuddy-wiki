...
subject=CN=Virtual Printer CA D55808BE
notBefore=... 2026 GMT
notAfter=... 2046 GMT
...
The characters after the name identify the Bambuddy instance that issued the certificate, so two instances' certificates can be told apart in the same file.
A certificate generated by an older Bambuddy has no such suffix and reads just `CN=Virtual Printer CA`.
...
- **Subject and Issuer aren't `Virtual`**: The `subject` and `issuer` should both start with `Virtual Printer CA`.
...
- **Two entries named plain `Virtual Printer CA`**: If you use two Bambuddy instances and appended both of their certificates, and both read exactly `CN=Virtual Printer CA` with no identifier after it, then only one of the two virtual printers will connect — the slicer finds the first certificate of that name and never tries the second.
    Both were issued by an earlier version of Bambuddy.
    Upgrade both instances, then on one of them delete `bbl_ca.crt` and `bbl_ca.key` from the `virtual_printer/certs/` directory in Bambuddy's data folder and restart it: a new certificate is issued with an identifier in its name.
    Remove that instance's old entry from `printer.cer` and append the new one, which you can download again from **Settings → Virtual Printer → Slicer certificate**.
